...
func (b *Budget) RemoveItem(description string) {
	for i := range b.Items {
		if b.Items[i].Description == description {
			// Start a appending items using +1 after the first item and go to the end of the list
...
func CreateBudget(month time.Month, max float32) (*Budget, error) {
	...
	// assign the value of the struct max
	// by using the "&" character
	newBudget = &Budget{Max: max}

	// lets also set a new month
...
func GetBudget(month time.Month) *Budget {
	...
	// grabbing values from report[month] and using
	// the ok to return the budget
